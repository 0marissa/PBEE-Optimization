...
% function, other than structural safety damage (for repair
% prioritization)
fnc_filters.affects_function = comp_ds_table.affects_envelope_safety | ...
...
fnc_filters.int_fall_haz_sf = comp_ds_table.int_falling_hazard & (strcmp(string(comp_ds_table.unit),'sf') | strcmp(string(comp_ds_table.unit),'each')); % Interior components with area feet units (or each, which is just lights, which we take care of with the fraction affected area, which is probably not the best way to do it)
...
fnc_filters.interior_function_sf = comp_ds_table.obstructs_interior_space & (strcmp(string(comp_ds_table.unit),'sf') | strcmp(string(comp_ds_table.unit),'each'));  
